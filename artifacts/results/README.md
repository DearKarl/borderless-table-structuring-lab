...
not included. The integrated application now includes native inference,
assembler003's consumer-format gate and a frozen evaluator adapter. A clone
still does not include private prediction caches or benchmark payloads.
A new device supplies its own licensed inputs and records a new execution;
the historical result is not automatically inherited by that execution.
